...
function synchronizeCrmToUmas(index, crmStudentsList) {
  ...
  request('https://crm.zoho.com/crm/private/json/Contacts/getRecords?newFormat=1&authtoken=36beb038e36a00d0bf8c22c8befd0c9b&scope=crmapi&' + INDEXES[index] + '&selectColumns=All', function (error, response, body) {
          ...
          var student = crmStudentsList[i];
            ...
            uplusModule.buscarPostulanteRut({'ADD_RUT' : addRut}, function (umasStudent) {
              ...
              if (umasStudent) {
                busy = false;
                i++;
                // console.log("UPDATE IN UMAS: ", crmStudentsList.indexOf(student));
  ...
  uplusModule.buscarMatriculados(function (students) {
    ...
    interval = setInterval(function () {
      ...
      if (student) {
        ...
        if (crmStudent) {
          console.log("STUDENT TO UPDATE => ", crmStudent['RUT']);
          ...
          console.log(student['ADD_RUT'] + " - " + student['FIRSTNAME']);
...
function insertCrmStudent(umasStudent) {
  var url = 'https://crm.zoho.com/crm/private/xml/Contacts/insertRecords?authtoken=36beb038e36a00d0bf8c22c8befd0c9b&scope=crmapi&newFormat=1&xmlData=' + buildCrmInsertStudent(umasStudent) + '&duplicateCheck=1';
...
function updateCrmStudent(umasStudent, crmStudent, id) {
  ...
  var url = 'https://crm.zoho.com/crm/private/xml/Contacts/updateRecords?authtoken=36beb038e36a00d0bf8c22c8befd0c9b&scope=crmapi&newFormat=1&xmlData=' + buildCrmUpdateStudent(umasStudent, crmStudent) + '&id=' + id;
  request(url, function (error, response, body) {
    console.log(body);
  })
}
