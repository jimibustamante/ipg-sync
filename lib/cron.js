...
function synchronizeCrmToUmas() {
  ...
  var inserts = 0,
    i = 0,
    updates = 0;
  request('https://crm.zoho.com/crm/private/json/Contacts/getRecords?newFormat=1&authtoken=36beb038e36a00d0bf8c22c8befd0c9b&scope=crmapi&fromIndex=1&toIndex=200&selectColumns=All', function (error, response, body) {
    if (error) {
      console.log("ERROR");
      console.log(error);
    }
    if (!error && response.statusCode == 200) {
      ...
      async.each(studentList, function (student, callback) {
        ...
        // if (student['First Name'].indexOf('Matriculado') >= 0  || student['First Name'].indexOf('Matriculado') >= 0 || student['Last Name'].indexOf('Matriculado') >= 0 || student['Last Name'].indexOf('Matriculado') >= 0) {
        //   console.log("\nJUAN PEREZ? \n");
        //   console.log(student);
        // }
